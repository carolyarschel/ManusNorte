...
import { detectConflicts, getISOWeek, DAY_NAMES, computeMonthlyOccupancy, remainingCapacity } from "@/lib/domain";
import { StatusBadge } from "@/components/ui/NorteUI";
import { isWorkingDay } from "@/lib/holidays";
import type { Consultant, Project } from "@/types/index";
  ...
  return (
    <div className="page-content">
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        <button className="btn btn-secondary btn-sm" onClick={() => exportCSV(consultants, projects)}>
          ⬇ Exportar CSV
        </button>
      </div>
      <div className="grid-4" style={{ marginBottom: 16 }}>
        ...
        <div className="stat-card">
          ...
          <button onClick={() => setShowOccFilter((v) => !v)} style={{ marginTop: 6, fontSize: 10, color: "var(--muted)", background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 3 }}>
            <span style={{ fontSize: 11 }}>⚙</span>
            {showOccFilter ? "fechar" : "filtrar consultores"}
            {!showOccFilter && excludedIds.size > 0 && (
              <span style={{ background: "var(--red)", color: "#fff", borderRadius: 8, padding: "1px 5px", fontSize: 9, fontWeight: 700 }}>{excludedIds.size}</span>
            )}
          </button>
          {showOccFilter && (
            <div style={{ marginTop: 8, borderTop: "1px solid var(--border)", paddingTop: 6, maxHeight: 160, overflowY: "auto" }}>
              {consultants.map((c) => {
                const excluded = excludedIds.has(c.id);
                return (
                  <label key={c.id} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, cursor: "pointer", padding: "2px 0", color: excluded ? "var(--muted)" : "var(--text)", textDecoration: excluded ? "line-through" : "none" }}>
                    <input type="checkbox" checked={!excluded} onChange={() => toggleConsultant(c.id)} style={{ cursor: "pointer", accentColor: "var(--red)" }} />
                    {c.name}
                  </label>
                );
              })}
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>capacidade total do time nos próximos 6 meses</div>
        </div>
          ...
          {projection.map((m, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                <div style={{ fontSize: 10, color, fontWeight: 700, whiteSpace: "nowrap" }}>{m.free}d livres</div>
                <div style={{ width: "100%", height: CHART_H, borderRadius: 4, overflow: "hidden", display: "flex", flexDirection: "column", border: `1.5px solid ${isNow ? color : "#e0e0e0"}`, boxShadow: isNow ? `0 0 0 2px ${color}28` : undefined }} title={`${m.label}: ${m.free}d livres de ${m.total}d (${Math.round(usedPct * 100)}% ocupado)`}>
                ...
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 10, fontWeight: isNow ? 700 : 400, color: isNow ? "var(--text)" : "var(--muted)" }}>{m.label}</div>
                  <div style={{ fontSize: 9, color: "var(--muted)", marginTop: 1 }}>{Math.round(usedPct * 100)}% ocup.</div>
                </div>
              </div>
            );
          })}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", marginTop: 12, paddingTop: 10, borderTop: "1px solid var(--border)", fontSize: 10, color: "var(--muted)" }}>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 12, height: 12, background: "#27ae6018", border: "1px solid #27ae60", borderRadius: 2, display: "inline-block" }} />Dias livres</span>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 12, height: 12, background: "#aaa", borderRadius: 2, display: "inline-block", opacity: 0.7 }} />Dias ocupados</span>
          <span style={{ marginLeft: "auto" }}>Cor: <span style={{ color: "#27ae60", fontWeight: 600 }}>verde</span> &gt;40% livre · <span style={{ color: "#e67e22", fontWeight: 600 }}>laranja</span> 15–40% · <span style={{ color: "#e74c3c", fontWeight: 600 }}>vermelho</span> &lt;15%</span>
        </div>
      </div>
      {conflicts.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
            <div className="card-title" style={{ margin: 0, color: "var(--red)" }}>Conflitos Detectados</div>
            <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 10, background: "#fdf1f0", color: "var(--red)", border: "1px solid #f5b7b1" }}>{conflicts.length}</span>
          </div>
          {conflictGroups.map((group) => (
            <div key={group.label} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: group.headerColor, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
                {group.label}
                <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: "var(--muted)", fontSize: 11 }}>— {group.description}</span>
              </div>
              {group.items.map((conflict, i) => {
                const sharedNames = conflict.sharedConsultants.map((id) => consultantMap.get(id)?.name ?? `#${id}`).join(", ");
                const days = conflict.sharedDays.map((d) => DAY_NAMES[d]).join(", ");
                return (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "9px 12px", borderRadius: 6, marginBottom: 6, background: group.rowBg, border: `1px solid ${group.rowBorder}` }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", marginTop: 5, flexShrink: 0, background: conflict.severity === "high" ? "#c0392b" : "#e67e22" }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
                        <span style={{ fontFamily: "var(--font-hd)", fontWeight: 800, color: "var(--red)", fontSize: 13 }}>{conflict.a.acronym}</span>
                        <StatusBadge status={conflict.a.status} />
                        <span style={{ color: "var(--muted)", fontSize: 12 }}>×</span>
                        <span style={{ fontFamily: "var(--font-hd)", fontWeight: 800, color: "var(--red)", fontSize: 13 }}>{conflict.b.acronym}</span>
      ...
      {(() => {
        const ROW_H = 36, HEADER_H = 30, LABEL_W = 170, MONTH_W = 90;
        const totalW = Math.max(300, ganttMonths.length * MONTH_W);
        return (
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <div style={{ padding: "20px 20px 12px" }}><div className="card-title">Linha do Tempo</div></div>
            <div style={{ display: "flex" }}>
              <div style={{ width: LABEL_W, flexShrink: 0, borderRight: "1px solid var(--border)", background: "var(--surface)", zIndex: 2 }}>
                <div style={{ height: HEADER_H, borderBottom: "1px solid var(--border)" }} />
                {activeProjects.map((p) => (
                  <div key={p.id} style={{ height: ROW_H, display: "flex", alignItems: "center", padding: "0 10px 0 20px", borderBottom: "1px solid var(--border)", overflow: "hidden" }}>
                    <span style={{ fontFamily: "var(--font-hd)", fontWeight: 800, color: "var(--red)", marginRight: 5, whiteSpace: "nowrap", fontSize: 13 }}>{p.acronym}</span>
                    <span style={{ fontSize: 11, color: "var(--muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.client}</span>
                  </div>
                ))}
                <div style={{ height: 12 }} />
              </div>
              <div ref={ganttScrollRef} style={{ flex: 1, overflowX: "auto", cursor: ganttDragging ? "grabbing" : "grab", userSelect: "none" }} onMouseDown={onGanttMouseDown} onMouseMove={onGanttMouseMove} onMouseUp={onGanttDragEnd} onMouseLeave={onGanttDragEnd}>
                <div style={{ display: "flex", width: totalW, height: HEADER_H, borderBottom: "1px solid var(--border)" }}>
                  {ganttMonths.map((m) => (<div key={m} className="gantt-month" style={{ width: MONTH_W, flexShrink: 0, height: HEADER_H }}>{m}</div>))}
                </div>
                {activeProjects.map((p) => (
                  <div key={p.id} style={{ height: ROW_H, position: "relative", width: totalW, borderBottom: "1px solid var(--border)" }}>
